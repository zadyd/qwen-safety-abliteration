"""
Step 4: Analyze results and generate visualizations (Qwen3.5-9B version).

Reads:
  - results_9b/abliteration_results.json  (per-layer Abliteration results)
  - results_9b/refusal_scores.json         (per-layer refusal direction scores)

Outputs:
  - results_9b/per_layer_refusal_drop.png   (bar chart)
  - results_9b/refusal_signal_heatmap.png   (heatmap)
  - results_9b/batch_group_refusal_drop.png (batch-group bar chart)
  - results_9b/findings.md                (summary report)
"""
import sys
import json
import matplotlib
...
def plot_per_layer_refusal_drop(results: dict, output_path: Path):
    ...
    ax.set_title("Qwen3.5-9B Abliteration: Per-Layer Refusal Signal Contribution", fontsize=13, fontweight="bold")
...
def plot_heatmap(results: dict, output_path: Path):
    ...
    # Reshape into 4-row grid: 8 columns for 32 layers
    rows = 4
    cols = 8
    grid = []
    for r in range(rows):
    ...
    ax.set_xticklabels([str(i) for i in range(cols)], fontsize=9)
    ...
    ax.set_title("Qwen3.5-9B Refusal Signal Heatmap", fontsize=12, fontweight="bold")
...
def plot_batch_group_results(results: dict, output_path: Path):
    ...
    ax.set_title("Qwen3.5-9B Batch Abliteration: Group-Level Refusal Disruption", fontsize=12, fontweight="bold")
